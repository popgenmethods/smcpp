...
def test_d():
    ...
    s = np.array([0.3] * K)
    n = 10
    sfs, _, jac = _pypsmcpp.sfs((a, b, s), n, num_samples, 0., np.inf, NTHREAD, 1.0, jacobian=True, seed=1)
    eps = 2e-8
    I = np.eye(K)
    for ind in (0, 1):
        for k in range(K):
            ...
            print(args)
            la, lb = args
            sfs2, rsfs2 = _pypsmcpp.sfs((la, lb, s), n, num_samples, 0., np.inf, NTHREAD, 1.0, jacobian=False, seed=1)
            for i in (0, 1):
                for j in range(n + 1):
                    jaca = jac[i, j, ind, k]
                    j1 = sfs2[i, j]
