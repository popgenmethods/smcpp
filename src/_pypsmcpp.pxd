...
cdef extern from "conditioned_sfs.h":
    ...
    void cython_calculate_sfs(const vector[double] a, const vector[double] b, const vector[double] s,
            int n, int S, int M, const vector[double] &ts, 
            const vector[double*] &expM, double tau1, double tau2, int numthreads, double theta, 
            double* outsfs)
    void cython_calculate_sfs_jac(const vector[double] a, const vector[double] b, const vector[double] s,
            int n, int S, int M, const vector[double] &ts, 
            const vector[double*] &expM, double tau1, double tau2, int numthreads, double theta, 
            double* outsfs, double* outjac)

cdef extern from "transition.h":
    void cython_calculate_transition(const vector[double] a, const vector[double] b, const vector[double] s, 
            const vector[double] hidden_states, double rho, double* outtrans)
    void cython_calculate_transition_jac(const vector[double] a, const vector[double] b, const vector[double] s, 
