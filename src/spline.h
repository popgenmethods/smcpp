...
/* Further modifications to this code: add template support for autodifferentiation */
...
template <typename T>
T & band_matrix::operator () (int i, int j)
{
...
template <typename T>
T band_matrix::operator () (int i, int j) const
{
    int k=j-i;       // what band is the entry
    assert( (i>=0) && (i<dim()) && (j>=0) && (j<dim()) );
    assert( (-num_lower()<=k) && (k<=num_upper()) );
...
template <typename T>
T band_matrix::saved_diag(int i) const
{
    assert( (i>=0) && (i<dim()) );
    return m_lower[0][i];
}
template <typename T>
T & band_matrix::saved_diag(int i)
{
    assert( (i>=0) && (i<dim()) );
    return m_lower[0][i];
}
...
// solves Ly=b
template <typename T>
std::vector<T> band_matrix::l_solve(const std::vector<T>& b) const
{
...
// solves Rx=y
template <typename T>
std::vector<T> band_matrix::r_solve(const std::vector<T>& b) const
{
    assert( this->dim()==(int)b.size() );
    std::vector<T> x(this->dim());
    int j_stop;
...
template <typename T>
std::vector<T> band_matrix::lu_solve(const std::vector<T>& b,
...
template <typename T>
void spline::set_points(const std::vector<T>& x,
