...
class Posterior(command.Command):
    ...
    def init_parser(self, parser):
        ...
        parser.add_argument("--thinning", type=int, default=1,
                            help="emit full SFS only every <k>th site", metavar="k")
        ...
        parser.add_argument("--matrix", "-m", metavar="file.npz",
                     help="Save raw matrix to file (numpy.savez() format)")
        ...
        parser.add_argument("data", type=str, help="SMC++ data set to decode")
        parser.add_argument("output", metavar="posterior.{png,pdf}", 
                            help="location to save heatmap")
    ...
    def main(self, args):
        ...
        hidden_states = _smcpp.balance_hidden_states(
            m.distiguished_model, args.M)
        obs = data['obs'][0]
        ...
        if args.thinning is not None:
            obs = util.compress_repeated_obs(
                _smcpp.thin_data(obs, args.thinning, 0))
        if npop == 1:
        ...
        for row, col in it.izip(obs, im.gammas[0].T):
            ...
            sp += an
        if args.matrix:
            np.savez_compressed(args.matrix, 'posterior'=gamma)
        # Plotting code
        fig, ax = plt.subplots()
        x = np.cumsum(obs[:, 0])
