...
class JointCSFS(object):
    def __init__(self, n1, n2, a1, a2, model1, model2, split, hidden_states):
        ...
        self._jcsfs = np.zeros([len(hidden_states) - 1, self._a1 + 1, self._n1 + 1, self._a2 + 1, self._n2 + 1], dtype=object)
    ...
    def compute(self):
        ...
        self._jcsfs[:] = list(map(_parallel_helper, ((t1, t2) + args for t1, t2 in self._hs_pairs)))
        return self._jcsfs

def _parallel_helper(tup):
    t1, t2, split, *args = tup
    print(t1, t2)
    if t1 < t2 <= split:
        return _jcsfs_helper_tau_below_split(t1, t2, split, *args)
    elif split <= t1 < t2:
        return _jcsfs_helper_tau_above_split(t1, t2, split, *args)
    return
...
def _jcsfs_helper_tau_above_split(t1, t2, split, a1, a2, n1, n2, model1, model2, Rts1, Rts2, K):
    ...
    shim_model = PiecewiseModel(sp, ap, list(dlist))
    rsfs = _smcpp.raw_sfs(shim_model, n1 + n2 + 2, t1, t2, True).astype('object')
    S2 = np.arange(1, n1 + n2 + 1, dtype=float) / (n1 + n2 + 1)
    S0 = 1. - S2
    M0 = MoranEigensystem(n1 + n2, 0)
    M10 = MoranEigensystem(n1, 0)
    M11 = MoranEigensystem(n1, 1)
    eM10 = M10.expm(Rts1)
    eM11 = M11.expm(Rts1)
    eM12 = eM10[::-1, ::-1]
    M2 = MoranEigensystem(n2)
    eM2 = M2.expm(Rts2)
    ...
    for tau, Rtau in _smcpp.random_coal_times(model1, t1, t2, K):
        ...
        m1s[tm1] = tau
        m1s = m1s[tm1:]
        m1a = model1.stepwise_values()[tm1:]
