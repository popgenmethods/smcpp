...
def sfs(n, M, N0, theta, demography=[]):
    ...
    output = check_output(
            """%s | grep SFS | tail -n+2 | cut -f2- -d' ' | Rscript -e 'colMeans(read.table(file("stdin")))'""" % cmd, shell=True)
    return np.array([float(x) for x in output.split("\n")[1].strip().split(" ")])
    # output = sh.Rscript(sh.cut(
    #             sh.tail(sh.grep(scrm(*args, _piped=True), "^SFS", _ok_code=[0, 1], _piped=True), 
    #                 "-n+2", _piped=True), 
    #             '-f2-', '-d ', _piped=True),
    #         '-e', 'colMeans(read.table(file("stdin")))')
    # print(output)
